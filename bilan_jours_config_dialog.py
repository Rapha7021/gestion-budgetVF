...
class BilanJoursConfigDialog(QDialog):
    def __init__(self, parent, projet_id=None):
        ...
        self.setWindowTitle("Configuration du bilan des jours")
        ...
        generate_btn = QPushButton("Générer le Bilan des Jours")
        generate_btn.clicked.connect(self.generate_bilan_jours)
    ...
    def generate_bilan_jours(self):
        """Génère le bilan des jours"""
        ...
        # Préparer la configuration pour le bilan des jours
        config_data = {
        ...
        # Vérifier d'abord s'il y a des données avant d'ouvrir le dialogue
        try:
            # Vérification préalable des données
            if not self.check_data_exists(config_data):
                QMessageBox.warning(self, "Aucune donnée", 
                                   "Aucune donnée de jours travaillés n'a été trouvée pour les projets et années sélectionnés.\n\n"
                                   "Vérifiez que :\n"
                                   "- Les projets sélectionnés ont des données de temps de travail\n"
                                   "- Les années sélectionnées correspondent aux données existantes\n"
                                   "- Les données ont été correctement saisies dans la gestion du budget")
                return  # Ne pas ouvrir le dialogue, rester sur la configuration
            
            # Si on arrive ici, il y a des données, on peut ouvrir le dialogue
            from bilan_jours_display import show_bilan_jours
            result = show_bilan_jours(self.parent, config_data)
            
        except ImportError as e:
            QMessageBox.critical(self, "Erreur", f"Impossible d'importer le module bilan_jours_display: {str(e)}")
        except Exception as e:
            QMessageBox.critical(self, "Erreur", f"Erreur lors de l'affichage du bilan des jours: {str(e)}")
    ...
    def check_data_exists(self, config_data):
        """Vérifie s'il existe des données de jours travaillés pour la configuration donnée"""
        ...
        try:
            # Vérifier s'il y a au moins une entrée avec des jours > 0
            placeholders_projects = ','.join('?' * len(project_ids))
            ...
            cursor.execute(query, project_ids + years)
...
def show_bilan_jours_config_dialog(parent, projet_id=None):
    """Fonction d'entrée pour afficher la configuration du bilan des jours"""
    dialog = BilanJoursConfigDialog(parent, projet_id)
    return dialog.exec()